...
public class PreSpriderUtil {
    ...
    /**
     * 对于同一个ip具体实现
     * 获取当前请求的ip的值，根据键值去获取redis缓存数据苦中取出
     * 该值，若无则添加进redis中说明该请求第一次不做判断
     * 若有值去除上词请求的时间  和本次的的请求时间进行相减  若小于0.5s则 添加1
     * 如果计数器达到 3次则表明这个是个可疑的ip 返回fasle
     * 所有的数据都都在redis中
     * 过滤爬虫
     * @return
     */
